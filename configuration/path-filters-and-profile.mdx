...
- Anywhere `block_merge_at` would otherwise become a problem (it's not configurable; defaults are sane)
...
2. **Per-role specialists** fire on what remains, with `profile` shifting their flag-worthiness floor.
3. **`deterministicChecks`** runs on added diff lines (also after `pathFilters` — filtered files don't get regex'd either).
