...
The `<role>` token in `rules` is always one of `logic`, `security`, `performance`, `tests`. These are the internal specialist role names; their public-facing brand counterparts are Athena (logic), Argus (security), Hermes (performance), and Themis (tests) respectively. A fifth specialist, the synthesizer **Zeus**, runs after the four domain specialists to unify their findings.
...
```json sigilix.json
{
  ...
  "commands": {
    "review":   { "prompt": "addendum", "model": "model-id-from-allowlist" },
    "improve":  { "prompt": "addendum" },
    "describe": { "prompt": "addendum", "model": "model-id-from-allowlist" }
  },
  "depVulns":      { "enabled": true },
...
{
  "commands": {
    "review":   { "model": "model-id-from-allowlist" },
    "describe": { "prompt": "Always start changelogs with the user-facing impact, never the implementation." }
  }
}
```

Valid model names are the model identifiers on Sigilix's current allowlist. The allowlist tracks the providers and model versions in production and changes over time, so run `/sigilix help` or check the live command-override docs for the current set rather than pinning a model ID here. Unrecognized models fall back to the command's default with a warning.
