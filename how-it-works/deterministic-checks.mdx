...
Sigilix has a **deterministic checks layer** that runs *before* the four-specialist ensemble. Matches surface as authoritative findings that the synthesizer treats as ground truth, not as another opinion to weigh.
...
flowchart LR
    ...
    classDef core fill:#C6234A,stroke:#C6234A,color:#FFFFFF,stroke-width:2px
...
Because the layer runs first, its findings are **injected into the specialist prompts as facts**. When Warden gets the prompt, it already knows "the secret scanner flagged an AKIA key on line 142." Warden doesn't have to rediscover the secret; it can focus on the surrounding code path (how does the key get used? is it logged anywhere?).
