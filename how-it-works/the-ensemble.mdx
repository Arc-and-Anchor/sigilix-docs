...
2. **Specialists run in parallel** — Glyph, Warden, Pulse, and Weave receive the same diff with different prompts and different primary models. They can't see each other's findings. Each runs with a per-attempt timeout cap and a cross-provider fallback so a single provider outage never silences a specialist.
