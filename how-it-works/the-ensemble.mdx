...
Sigilix's architecture is different. Five specialists run per review: four domain specialists in parallel — **Athena** (logic/architecture), **Argus** (security), **Hermes** (performance), **Themis** (tests/semantics) — with focused prompts and role-tuned models, unified by **Zeus**, the synthesizer that arbitrates between them. Before the LLM specialists fire, a pre-LLM layer of deterministic checks — secret scanning, AST rule packs, user-defined regex rules — extracts cheap signal from the diff.
...
flowchart TD
    ...
    athena["Athena<br/>Architecture"]
    argus["Argus<br/>Security"]
    hermes["Hermes<br/>Performance"]
    themis["Themis<br/>Semantics"]
    zeus["Zeus<br/>Synthesizer"]
...
2. **Domain specialists run in parallel** — Athena, Argus, Hermes, and Themis receive the same diff plus the deterministic findings as authoritative facts in their context. Each specialist has a different prompt and a model tuned to its role, and can't see the other specialists' findings. Each runs with a size-scaled budget and a cross-provider fallback that protects against same-family provider outages.
3. **Findings flow into Zeus** — the synthesizer sees all four streams plus the deterministic findings plus the diff itself.
4. **Zeus deduplicates, calibrates, and renders** — overlapping findings collapse into one; severity shifts based on agreement; review memory adjusts category-level flag-worthiness; each surviving finding earns a proof-tier receipt; the final verdict is decided.
5. **One comment is posted** — single GitHub review with the Zeus summary at the top and inline findings below.
...
Each specialist runs a model tuned to its role — a reasoning-heavy model for logic (Athena's architectural chain-of-thought), faster high-volume models for security and tests (Argus, Themis), a throughput-tuned model for performance (Hermes), and a calibration-strong model for synthesis (Zeus). The specific model behind each role is tuned over time from telemetry; the docs describe the role, not a model ID that churns.
...
Sigilix's synthesizer cross-references findings with the source code. If Argus flags a SQL injection at line 42 but Zeus's structural-provenance check shows the parameter actually passes through a parameterized-query helper, the finding is suppressed before it reaches you.
...
When multiple specialists flag the same code, that's a strong signal. Zeus escalates the severity in those cases:
...
- **Specialist + Zeus's structural check confirms** → Critical
...
Zeus deduplicates relentlessly. If Argus and Hermes both flag the same loop, you see one comment, not two. If a finding is a duplicate of one already posted on a prior SHA, you see it once.
...
<CardGroup cols={2}>
  ...
  <Card title="Synthesizer" icon="git-merge" href="/how-it-works/synthesizer">
    Zeus's pipeline: collect → cross-reference → calibrate → render, inside the believability pipeline.
  </Card>
</CardGroup>
