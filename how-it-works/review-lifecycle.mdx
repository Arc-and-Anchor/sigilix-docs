...
10. Quorum check
   ↓
11. Synthesizer (Zeus) — grounding gate, proof-tier receipts, memory calibration
   ↓
...
- **Default** → all four domain specialists (Athena, Argus, Hermes, Themis)
...
|---|---|---|
| Athena | `logic` | Reasoning-heavy primary, cross-provider fallback |
| Argus | `security` | Fast high-volume primary, cross-provider fallback |
| Hermes | `performance` | Throughput-tuned primary, cross-provider fallback |
| Themis | `tests` | Fast broad-coverage primary, cross-provider fallback |
...
See the [Synthesizer page](/how-it-works/synthesizer) for the four sub-stages. Zeus runs a calibration-strong model with a cross-provider fallback. This is where the **grounding gate** runs and each surviving finding earns its [proof-tier receipt](/how-it-works/confidence-scoring).
...
<CardGroup cols={2}>
  <Card title="The Dispatcher" icon="route" href="/how-it-works/the-dispatcher">
    One webhook → many pipelines: how each event is routed.
  </Card>
  <Card title="Confidence & Proof Tiers" icon="chart-bar" href="/how-it-works/confidence-scoring">
