...
The pr-review webhook ingest path runs through a Durable Object ([ARC-190](/operations-and-releases/durable-objects)) for per-PR serialization across regions, so the same PR can never have two concurrent in-flight reviews.
...
- **Idempotency.** Re-firing the same webhook never produces a duplicate review (Durable-Object serialization, ARC-190).
...
- **Cross-provider resilience.** Per-attempt timeout caps plus cross-provider fallbacks ensure a same-family provider outage never silences a specialist (Phase 4a.4).
