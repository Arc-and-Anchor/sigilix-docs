...
The `specialist` field is the **role** name (`logic`, `security`, `performance`, `tests`). The public-facing brand names — Glyph, Warden, Pulse, Weave — are rendered into the final review comment by the synthesizer.
...
### Specialist 503s + per-attempt timeouts (Phase 4a.4)

If one specialist's model returns 503 (overloaded) or exceeds its per-attempt timeout, Sigilix retries once on the primary, then fires the cross-provider fallback. Primary + fallback run on independent infrastructure so a same-family provider outage can't silence the same role twice.

If all attempts for a specialist fail, the specialist's findings are skipped — but Core still synthesizes from the remaining specialists. The verdict is still posted with a footnote: `_3 of 4 specialists succeeded._`
