...
<!-- sigilix-meta: {
  ...
  "specialists": {
    "logic":       { "specialist": "Athena", "outcome": "ok",       "ms": 21300 },
    "security":    { "specialist": "Argus",  "outcome": "fallback", "ms": 28100 },
    "performance": { "specialist": "Hermes", "outcome": "ok",       "ms": 19800 },
    "tests":       { "specialist": "Themis", "outcome": "ok",       "ms": 16400 }
  },
  "synthesis":   { "specialist": "Zeus", "outcome": "ok", "ms": 9800 },
...
The four domain specialists — **Athena** (logic/architecture), **Argus** (security), **Hermes** (performance), **Themis** (tests) — run in parallel and are unified by **Zeus**, the synthesizer that runs after them. The manifest records the role key (`logic` / `security` / `performance` / `tests` / `synthesis`) alongside each specialist's display name.
...
In the rendered review, this is the `[Athena]` / `[Argus]` / `[Hermes]` / `[Themis]` prefix on each inline finding, plus an evidence badge (`[Trivy via SARIF]`, `[Secret scanner]`, etc.) when the finding came from an external channel.
