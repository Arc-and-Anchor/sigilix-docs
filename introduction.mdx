...
Every pull request is read in parallel by four domain specialists — **Athena** (logic and architecture), **Argus** (security), **Hermes** (performance), and **Themis** (tests) — and then unified by **Zeus**, the synthesizer that cross-checks their findings, deduplicates noise, calibrates severity, and posts a single coherent review.
...
<CardGroup cols={2}>
  <Card title="Athena · Logic & Architecture">
...
1. A **Zeus summary** — what changed, what concerns survived dedup, and whether the verdict is *Approved* or *Request changes*.
2. **Inline findings** anchored to specific lines, tagged by specialist (Athena, Argus, Hermes, Themis) and severity (Critical / Warning / Info), each carrying its proof-tier pill.
