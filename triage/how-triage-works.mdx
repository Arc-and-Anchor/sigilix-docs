...
<Steps>
  ...
  <Step title="Dispatcher routes to triage">
    The [dispatcher](/how-it-works/the-dispatcher) classifies the event and hands it to the triage pipeline rather than the full ensemble. This is the cost-and-latency isolation the dispatcher exists for: triage gets a path sized to its job, not a five-specialist review.
  </Step>
  <Step title="Retrieve the relevant code">
  ...
  <Step title="Reason, grounded in that context">
  ...
  <Step title="Emit a structured result">
  ...
  <Step title="Post as a single comment">
...
<CardGroup cols={3}>
  ...
  <Card title="Symbol-aware expansion" icon="magnifying-glass">
...
<AccordionGroup>
  <Accordion title="Why grounding matters most in triage">
  ...
  <Accordion title="CI triage enforces it">
  ...
  <Accordion title="Shared with the believability pipeline">
