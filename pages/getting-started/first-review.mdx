...
description: "What every Sigilix review contains — Harmonia summary, inline findings, severity badges, proof-tier pills, suggested patches — and how to read it."
---
...
<style>{`
  ...
  .sigilix-verdict-pill { padding: 0.125rem 0.625rem; border-radius: 999px; font-size: 0.75rem; font-weight: 600; color: #f1140d; background: rgba(241,20,13,0.12); border: 1px solid rgba(241,20,13,0.35); white-space: nowrap; }
  .sigilix-synth-note { margin: 0 0 1rem !important; }
  .sigilix-sev-critical { background: #f1140d !important; color: #fff !important; }
  ...
  .sigilix-diff-link { display: inline-block; margin-top: 0.5rem; font-family: ui-monospace, monospace; font-size: 0.8125rem; color: #f1140d; text-decoration: none; border-bottom: 1px dashed rgba(241,20,13,0.4); }
  .sigilix-diff-link:hover { border-bottom-color: #f1140d; }
`}</style>
...
<div className="sigilix-review-chrome">
    ...
    <details>
<summary>Argus · Security · Critical · VERIFIED</summary>
...
<a href="#" className="sigilix-diff-link">View diff →</a>
      ...
      <details>
<summary>Iris · Performance · Warning · GROUNDED</summary>
...
<a href="#" className="sigilix-diff-link">View diff →</a>
      ...
      <details>
<summary>Eunomia · Tests · Info · GROUNDED</summary>
...
The first paragraph is from **Harmonia** (the synthesizer). It tells you:
- How many findings survived deduplication
- How many specialists contributed
- How many require action before merge

This is the only block worth reading if you're triaging fast. If Harmonia says "0 findings," skim and move on. If Harmonia says "1 critical," that's your thing.

## The inline findings

Each finding is collapsed by default and lives on a specific line in the diff. Click to expand and read:

- **Specialist tag** — Metis, Argus, Iris, Eunomia (Harmonia never posts inline findings — it only synthesizes)
...
Sigilix uses three severity levels. Calibration is done by Harmonia during synthesis:
...
A finding's severity can shift up or down during synthesis if multiple specialists flag the same code (escalation) or if Harmonia judges the finding a false positive (suppression).
