...
<!-- sigilix-meta: {
  ...
  "specialists": {
    "logic":       { "specialist": "Logic", "outcome": "ok",       "ms": 21300 },
    "security":    { "specialist": "Security",  "outcome": "fallback", "ms": 28100 },
    "performance": { "specialist": "Performance", "outcome": "ok",       "ms": 19800 },
    "tests":       { "specialist": "Tests", "outcome": "ok",       "ms": 16400 }
  },
  "synthesis":   { "specialist": "Synthesizer", "outcome": "ok", "ms": 9800 },
  "evidence":    { "sarif": 3, "depVulns": 1, "secrets": 0 },
  "proofTiers":  { "verified": 1, "reproduced": 0, "grounded": 4, "model": 0 },
...
The four domain specialists — **Logic & Architecture**, **Security**, **Performance**, **Tests** — run in parallel and are unified by the **synthesizer** that runs after them. The manifest records the role key (`logic` / `security` / `performance` / `tests` / `synthesis`) alongside each specialist's display name.

<Callout type="info">
  Each specialist runs a Sigilix-tuned model matched to its role — a reasoning-heavy model for logic, a faster high-volume model for security — each with a cross-provider fallback so one provider's outage can't silence a specialist. The manifest commits to the durable contract: which specialist ran, whether it fell back, and the proof tier each finding earned.
</Callout>
...
In the rendered review, this is the `[Logic]` / `[Security]` / `[Performance]` / `[Tests]` prefix on each inline finding, plus an evidence badge (`[Trivy via SARIF]`, `[Secret scanner]`, etc.) when the finding came from an external channel.
...
| Tier | Pill | Meaning |
|---|---|---|
| Verified | **VERIFIED** | The claim carries a signed receipt from an independent system — a scanner in your own CI confirmed the same finding. The strongest tier: two unrelated witnesses agree. |
| Reproduced | **REPRODUCED** | The claim was demonstrated by execution — Sigilix's sealed sandbox re-ran the cited code and reproduced the behavior. A finding that says "this diverges on the boundary case" was shown to diverge. |
| Grounded | **GROUNDED** | The finding is anchored to cited code or deterministic evidence (a SARIF row, a CVE record, a quoted diff span). The claim is tied to something verifiable in the PR. |
...
3. **Execution / receipt.** Where a claim is checkable — and a check is available — it is executed or matched against a signed receipt. Execution in Sigilix's sealed sandbox promotes the finding to REPRODUCED; corroboration by a receipt from your own CI promotes it to VERIFIED. See [Verification & the Trust Ledger](/how-it-works/verification).
