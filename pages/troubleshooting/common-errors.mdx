...
If a Sigilix review's footer says `_3 of 4 specialists succeeded._`, one of the four domain specialists — Logic (logic), Security (security), Performance (performance), or Tests (tests) — failed. This usually means:
...
The review still posts with the available specialists; the synthesizer synthesizes whatever returned. Sigilix doesn't retry the missing specialist on the same SHA — push a new commit to trigger a fresh review.
