---
title: "Authentication"
description: "API authentication via OAuth2 personal access tokens. Required scopes per endpoint."
---

import { Cards } from 'nextra/components'

# Authentication

Sigilix itself is live and self-serve, but the public **REST API is not yet generally available**. This page documents the planned authentication model; endpoints will land in subsequent releases.

## Authentication

All API requests use a personal access token (PAT) in the `Authorization` header:

```bash
curl https://api.sigilix.ai/v1/reviews \
  -H "Authorization: Bearer sgx_pat_xxxxxxxxxxxx"
```

PATs are planned to be minted on the [Sigilix dashboard](https://sigilix.ai/dashboard) — dashboard PAT minting is **not yet available**. They're scoped to a single Sigilix installation — one PAT per GitHub org.

## Scopes

PATs have four scopes:

| Scope | Endpoints |
|---|---|
| `reviews:read` | List and fetch past reviews |
| `findings:read` | Fetch individual findings + suggested patches |
| `webhooks:write` | Configure outbound webhooks for `review.completed` events |
| `config:write` | Modify `sigilix.json` programmatically (CI integration) |

Choose the minimum scopes you need. The dashboard shows which endpoints each scope unlocks.

## Rate limits

API requests are rate-limited separately from review limits. API limits are set **per account** rather than by a published tier. Every response carries the current window state in headers:

```text
X-RateLimit-Limit: 1000
X-RateLimit-Remaining: 982
X-RateLimit-Reset: 2026-05-05T04:00:00Z
```

When you hit the limit, the API returns 429 with a `Retry-After` header.

## OAuth2 (coming)

For server-to-server integrations (e.g., a third-party platform integrating with Sigilix on behalf of customer orgs), an OAuth2 flow is on the roadmap.

## Read next

<Cards>
<Cards.Card title="Endpoints" href="/api-reference/endpoints">
    The reviews and findings endpoints (pre-GA).
  </Cards.Card>
<Cards.Card title="Webhooks" href="/api-reference/webhooks">
    Outbound webhook events Sigilix can fire on your endpoints.
  </Cards.Card>
</Cards>
