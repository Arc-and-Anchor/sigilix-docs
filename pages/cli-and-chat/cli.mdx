...
On paid tiers, the CLI does not lock you to one provider. Connect your own provider key and pay that provider's usage directly. Your model still fetches Sigilix's verified context layer, so it inherits the precision without re-deriving the repo.
