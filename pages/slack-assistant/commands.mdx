...
| `@Sigilix review <PR or branch>` | Runs the full Logic / Security / Performance / Tests + the synthesizer ensemble. |
...
Run the full ensemble on demand. The four specialists — **Logic** (logic & architecture), **Security** (security), **Performance** (performance), **Tests** (tests) — run in parallel, unified by **the synthesizer**, exactly as on an automatic review. See [The Ensemble](/how-it-works/the-ensemble).
