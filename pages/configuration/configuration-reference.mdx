...
The `<role>` token in `rules` is always one of `logic`, `security`, `performance`, `tests` — the four domain specialists, surfaced in reviews as **Logic** (`logic`), **Security** (`security`), **Performance** (`performance`), and **Tests** (`tests`). A fifth specialist, the **synthesizer**, runs after the four to unify their findings; it has no `rules` key of its own.
...
<Tabs items={['sigilix.json', 'sigilix.yaml']}>

<Tabs.Tab>

```json filename="sigilix.json"
{
