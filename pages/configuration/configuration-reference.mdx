...
The `<role>` token in `rules` is always one of `logic`, `security`, `performance`, `tests` — the four domain specialists. A separate **synthesizer** runs after them to unify their findings; it is not one of the four and has no `rules.<role>` token.
