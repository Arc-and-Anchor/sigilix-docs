...
|---|---|---|
| `logic` | Logic | Logic, layering, dependency rules, dead code |
...
2. **Signal clarity.** When you write "All POST routes need CSRF" in `guidance`, the performance specialist reads it too and now has noise to discount. Put it in `rules.security` and only Security sees it.
