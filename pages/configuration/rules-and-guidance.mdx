...
|---|---|---|
| `logic` | Metis | Architecture, layering, dependency rules, dead code |
| `security` | Argus | Auth boundaries, input validation, secret leakage |
| `performance` | Iris | N+1, hot paths, memory, big-O regressions |
| `tests` | Eunomia | Naming, semantics, test coverage, branch reachability |
...
2. **Signal clarity.** When you write "All POST routes need CSRF" in `guidance`, the performance specialist reads it too and now has noise to discount. Put it in `rules.security` and only Argus sees it.
