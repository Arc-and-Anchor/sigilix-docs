...
|---|---|---|
| Logic | `logic` | Reasoning-heavy primary, cross-provider fallback |
| Security | `security` | Fast high-volume primary, cross-provider fallback |
| Performance | `performance` | Throughput-tuned primary, cross-provider fallback |
| Tests | `tests` | Fast broad-coverage primary, cross-provider fallback |
...
Sigilix posts the review via `POST /repos/:owner/:repo/pulls/:number/reviews` with `event: APPROVE | REQUEST_CHANGES`, the body containing synthesizer summary, and `comments: []` for inline findings.
