...
- **Review memory.** Relevant [Conversational Learnings](/memory/conversational-learnings) and category calibration for this repo.
...
Each specialist runs a Sigilix-tuned model matched to its role, refined over time from telemetry. If a specialist's primary model fails (transient 503/429 or budget exhaustion), retry once. If retries are exhausted, the cross-provider fallback fires once. If the fallback also fails, that specialist's contribution is skipped.
...
Cross-PR [review memory](/memory/review-memory) feeds into this stage to calibrate severity per-category and apply learned rules.
...
- **Feedback-signal capture** — PR-thread reactions, "Commit suggestion" applies, reply text matching accept/dismiss patterns. Feeds the cross-PR [review memory](/memory/review-memory).
