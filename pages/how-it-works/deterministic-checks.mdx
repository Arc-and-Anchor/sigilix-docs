...
flowchart LR
    ...
    zeus["the synthesizer<br/>Synthesizer"]
...
Because the layer runs first, its findings are **injected into the specialist prompts as facts**. When Security gets the prompt, it already knows "the secret scanner flagged an AKIA key on line 142." Security doesn't have to rediscover the secret; it can focus on the surrounding code path (how does the key get used? is it logged anywhere?).
...
When the synthesizer synthesizes:
