...
As of writing, the production deployment has `PrReviewRunDO` registered with `mode=off` — the DO is live and recording, but doesn't yet drive review behavior. Shadow rollout on Sigilix's own repo is planned for ~2026-05-14; full `post` cutover ~2026-05-21; legacy KV path removal ~2026-06-21.

This staged rollout is deliberate. A bug in the DO that silently drops reviews would be terrible; running shadow alongside the legacy path catches mismatches before cutover.
...
DOs are billed per request and per GB-second of storage. For Sigilix the dominant cost is **requests**: each webhook is one request. Storage per PR is small (a manifest is ~1 KB; feedback signals are a few KB per PR).
...
<CardGroup cols={2}>
  ...
  <Card title="Review Provenance" icon="file-signature" href="/evidence-and-provenance/review-provenance">
